...
## 三条原理（技术科学层 — 覆盖下方25条工程实践层规则）

以下三条原理提炼自所有历史事故的共性根因。新的事故发生时，先判断违反哪条原理，再查对应规则。

### 原理1：状态一致性
任何涉及状态变更的操作（disabled、className、running、innerHTML、subLevel），必须保证变更后所有关联状态一致。

**覆盖规则**：规则11（中途换档）、规则12（disabled opacity）、规则13（style.cssText）、规则23（stopGame→startGame）

**检查清单**：
- 改了一个状态变量后，所有依赖它的 DOM/变量是否同步？
- 禁用按钮后，`:disabled` 伪类的 CSS 是否改变了视觉样式？
- 临时颜色反馈是否用 `style.cssText` 而非 `className` 替换？

### 原理2：数据完整性
任何处理输入/输出的函数（tokenize、genOptions、renderCategories、题库生成），必须保证不丢不增。

**覆盖规则**：规则17（_tokenize 不丢字符）、规则18（不添加设计文档外内容）、规则21（选项随机排序）、规则22（再玩一次直接重新开始）
...
### 原理3：CSS 布局确定性
任何 CSS 布局规则，必须显式覆盖所有影响尺寸的属性（width、margin、aspect-ratio），不依赖隐式默认值。

**覆盖规则**：规则5（aspect-ratio）、规则15（容器宽度匹配子面板）、规则19（grid margin auto 坍塌）

**检查清单**：
- `margin: auto` 的 grid 元素是否设置了 `width: 100%`？
- 容器 `max-width` 是否紧贴子元素总宽？
- 正方形元素是否用了 `aspect-ratio: 1`？
...
| 保留项 | 对应规则 | 原因 |
|--------|---------|------|
| 音频系统约束 | 规则6、规则7、规则25 | 音频 TTS/预录音频/整词生成是领域特殊约束 |
| 分类互斥 | 规则8 | 业务逻辑约束，非通用原理 |
| 横竖屏策略 | 规则10、规则16 | 视觉/听觉游戏有不同布局策略 |
| 静态资源路径 | 规则24 | file:// 协议的特殊限制 |
| 浏览器缓存 | 规则9 | 部署环境的特殊约束 |
| JS 语法验证 | 规则1 | 工程基础设施 |
| 静态资源路径验证 | 规则2 | 工程基础设施 |
| CSS 选择器双向验证 | 规则3、规则4 | 工程基础设施 |
| taste-skill 设计规范 | 规则20 | 设计领域约束 |
